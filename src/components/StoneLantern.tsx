...
function useStoneMaterial(color: string) {
  return useMemo(() => {
    const mat = new THREE.MeshStandardMaterial({
      color: color,
      roughness: 0.9,
      flatShading: false, // Smooth shading for organic feel
    })
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        // Slightly reduced deformation for more subtlety
        transformed += normal * def * 0.003;
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        vec3 p = vWorldPos.xyz * 40.0; // Higher frequency for finer grain
        ...
        vec3 grainBump = (vec3(nX, nY, nZ) - 0.5) * 0.15; // Increased grain strength
        ...
        vec3 largeBump = vec3(dfdx, dfdy, dfdz) * 0.04; // Increased unevenness
...
export function StoneLantern(props: any) {
  ...
  // Geometry Profiles - using LatheGeometry with higher segments for rounder, organic forms
  const segments = 24;
